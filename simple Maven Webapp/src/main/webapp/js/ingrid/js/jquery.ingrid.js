 ...
 * 
 * Copyright (c) 2009 Matthew Knight (http://reconstrukt.com) Patrice
 * Blanchardie (http://www.inisos.fr)
 */
...
jQuery.fn.ingrid = function(o) {
	...
	var cfg = {
		...
		savedStateLoad : false, // when Ingrid is initialized, should it load
		// data from a previously saved state?
		initialLoad : false, // when Ingrid is initialized, should it load
		// data immediately?
		...
		/* sorting */
		sorting : true,
		colSortParams : [], // value to pass as sort param when header clicked
		// (i.e. '&sort=param') ex:
		// ['col1','col2','col3','col4']
		...
		// '&dir=desc)
		sortedCol : 'col1', // current data's sorted column (can be a key from
		// 'colSortParams', or an int 0-n (for n columns)
		...
		}, // called when page changed (loading finished)
		...
		scrollbarW : 20, // width allocated for scrollbar
		columnIDAttr : 'data-ingrid-colid', // attribute name used to groups TDs
		// in columns
		rowSelectedAttr : 'data-ingrid-selected', // attribute name for
		// selected rows
		ingridIDPrefix : '_ingrid', // prefix used to create unique IDs for
		// Ingrid
	...
	// create header table
	var cols = new Array();
	var h = jQuery('<table cellpadding="0" cellspacing="0"></table>').html(this.find('thead')).addClass(cfg.gridClass).addClass(cfg.headerClass).height(cfg.headerHeight).extend({
		cols : cols
	});
	// initialize columns
	h.find('th').each(
			function(i) {
				...
				var col_label = jQuery('<div />').html(jQuery(this).html()).css({
					float : 'left',
					display : 'block'
				}).css('-moz-user-select', 'none').css('-khtml-user-select', 'none').css('user-select', 'none').attr('unselectable', 'on');
				...
				if (cfg.sorting && cfg.isSortableCol(i)) {
					...
					var cls = (key == cfg.sortedCol || i == cfg.sortedCol) ? (cfg.sortedColDir == cfg.sortAscParam ? cfg.sortAscClass : cfg.sortDescClass) : (cfg.sortNoneClass);

					col_label.addClass(cls).click(
							function() {
								var dir = col_label.hasClass(cfg.sortNoneClass) ? cfg.sortDefaultDir : (col_label.hasClass(cfg.sortAscClass) ? cfg.sortDescParam : cfg.sortAscParam);
								...
								g.load(params, function() {
									var cls = col_label.hasClass(cfg.sortNoneClass) ? (cfg.sortDefaultDir == cfg.sortAscParam ? cfg.sortAscClass : cfg.sortDescClass) : (col_label
											.hasClass(cfg.sortAscClass) ? cfg.sortDescClass : cfg.sortAscClass);
	...
	var b = jQuery('<div />').html(jQuery('<table cellpadding="0" id="' + this.attr("id") + '" cellspacing="0"></table>').html(this.find('tbody')).width(h.width()).addClass(cfg.gridClass)).css(
			'overflow', 'auto').height(cfg.height);

	// resizable cols?
	// if so create a vertical resize divider, with unique ID
	if (cfg.resizableCols) {
		var z_sel = 'vertical-resize-divider-' + new Date().getTime();
		var z = jQuery('<div id="' + z_sel + '"></div>').css({
			...
			resizeStart : function(th, eventX) {
				...
				// when resizing, bind some listeners for mousemove & mouseup
				// events
				jQuery('body').bind('mousemove', {
					...
					// make sure cursor isn't trying to make column smaller than
					// minimum
	...
	if (cfg.paging) {
		...
		// total record count might not be passed in config, it's sometimes an
		// expensive hit to the DB
		var pv;
		if (cfg.totalRecords > 0) {
			pv = jQuery('<div />').addClass(cfg.pageViewingRecordsInfoClass).extend({
		...
		var p = jQuery('<div />').addClass(cfg.pageToolbarClass).height(cfg.pageToolbarHeight).width(b.width()).extend({
			setPage : function(p) {
	...
	// append & extend grid {g} with header {h}, body {b}, paging {p}, resize
	// handle {z}
	...
	g.extend({
				...
				success : function(result) {
					...
					if (cfg.dataType == 'html') {
						...
						if (jQuery(row).find('td').length == cfg.colWidths.length) {
							// setting width on first row keeps it from
							// "blinking"
							jQuery(row).find('td').each(function(i) {
							...
							// inconsistent results... too many (or too few)
							// columns returned
		...
		getSelectedRowIds : function() {
			...
			for (i = 0; i < rows.length; i++) {
				if (jQuery(rows[i]).attr('id'))
					row_ids.push(jQuery(rows[i]).attr('id'));
			}
			return row_ids;
		},
		...
		initStylesAndWidths : function() {
			...
			this.getRows().each(function(r) {
				...
				jQuery(this).find('td').each(function(i) {
					// column IDs & width
					// wrap the cell text in a div with overflow hidden, so
					// cells aren't stretched wider by long text
					var txt = jQuery(this).html();
					// alert(jQuery(this) + "," +
					// jQuery(this).attr(cfg.columnIDAttr,i));
					jQuery(this).attr(cfg.columnIDAttr, i).width(colWidths[i]).html(jQuery('<div />').html(txt).css('overflow', 'hidden'));
					// column colors
				...
				if (cfg.rowSelection == true) {
					jQuery(this).click(function() {
						...
						if (jQuery(this).attr(cfg.rowSelectedAttr) && jQuery(this).attr(cfg.rowSelectedAttr) == 'true') {
							...
							// jQuery(this).attr(cfg.rowSelectedAttr,
							// 'false').removeClass(cfg.rowSelectedClass);
							...
							// jQuery(this).attr(cfg.rowSelectedAttr,
							// 'true').addClass(cfg.rowSelectedClass);
	...
	/*
	 * console.log(this + ' ...is jQuery') console.log(this[0] + ' ...is a
	 * table') console.log(this.length + ' = total tables matched to selector')
	 */
	...
	return this.each(
			function(tblIter) {
				...
				/*
				 * console.log(this + ' ...is a table [' + tblIter + '] , id="' +
				 * jQuery(this).attr('id') + '"') console.log(g[0] + ' ...is the
				 * grid div html');
				 */
				...
				if (cfg.savedStateLoad && jQuery.cookie) {
					...
					if (!param_str) {
						...
						if (hash['sort'].toLowerCase() != 'undefined' && hash['dir'].toLowerCase() != 'undefined') {
							...
							// perhaps the sort param is a key, if so, get the
							// id for
							// that key
							...
							// perhaps we should bind an event to the column
							// <th>, like
							// setSort()?
							// (re-init sortable cols)
							var i2 = 0;
							...
							// all this prevents the column from being
							// style-less (and
							// blinking)
							g.getHeader(parseInt(colid)).find('div:first').addClass(cfg.sortNoneClass).removeClass(cfg.sortAscClass).removeClass(cfg.sortDescClass).addClass(
									params.dir == cfg.sortAscParam ? cfg.sortAscClass : cfg.sortDescClass).removeClass(cfg.sortNoneClass);
						}
